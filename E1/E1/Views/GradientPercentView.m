...
@implementation GradientPercentView
...
-(void)layoutSubviews
{
    ...
    NSLog(@"the current frame's origin.x is %f",self.frame.origin.x);
    NSLog(@"the current frame's origin.y is %f",self.frame.origin.y);
    NSLog(@"the current frame's width is %f",self.frame.size.width);
    NSLog(@"the current frame's height is %f",self.frame.size.height);
